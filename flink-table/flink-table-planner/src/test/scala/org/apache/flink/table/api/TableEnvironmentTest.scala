...
class TableEnvironmentTest {
  ...
  def testRegisterDataStream(
    env: StreamExecutionEnvironment, tableEnv: StreamTableEnvironment): Unit = {
  ...
  @Test
  @WithTableEnvironment
  def testExplainWithExecuteInsert(
    tEnv: StreamTableEnvironment,
    @TempDir tempDir: Path): Unit = {
    TestTableSourceSinks.createPersonCsvTemporaryTable(tEnv, tempDir, "MyTable")
  ...
  private def verifyTableEnvironmentExecutionExplain(
    tEnv: TableEnvironment, tempDir: Path): Unit = {
    TestTableSourceSinks.createPersonCsvTemporaryTable(tEnv, tempDir, "MyTable")
  ...
  def testAlterTableResetInvalidOptionKey(tableEnv: TableEnvironment): Unit = {
    ...
    assertThat(tableEnv
      .executeSql("explain plan for select * from MyTable where a > 10").getResultKind)
  ...
  def testExecuteSqlWithCreateAlterDropTable(tableEnv: TableEnvironment): Unit = {
    ...
    assertThat(tableEnv.getCatalog(tableEnv.getCurrentCatalog).get()
  ...
  def testExecuteSqlWithCreateAlterDropDatabase(tableEnv: TableEnvironment): Unit = {
    ...
    assertThat(tableResult1.getResultKind).isEqualTo(ResultKind.SUCCESS)
    assertThat(
      tableEnv.getCatalog(tableEnv.getCurrentCatalog).get().databaseExists("db1"))
      .isTrue
  ...
  def testExecuteSqlWithExplainSelect(tableEnv: TableEnvironment): Unit = {
    ...
    checkExplain(tableEnv, "explain plan for select * from MyTable where a > 10",
  ...
  def testExecuteSqlWithUnsupportedExplain(tableEnv: TableEnvironment): Unit = {
    ...
    testUnsupportedExplain(
      tableEnv, "explain plan including all attributes for select * from MyTable")
    testUnsupportedExplain(tableEnv, "explain plan with type for select * from MyTable")
    testUnsupportedExplain(
      tableEnv, "explain plan without implementation for select * from MyTable")
  ...
  private def validateShowModules(
    tableEnv: TableEnvironment, expectedEntries: (String, java.lang.Boolean)*): Unit = {
    ...
    assertThat(showFullModules.getResolvedSchema).isEqualTo(ResolvedSchema.physical(
  ...
  private def checkListModules(
    tableEnv: TableEnvironment, expected: String*): Unit = {
  ...
  private def checkListFullModules(
    tableEnv: TableEnvironment, expected: (String, java.lang.Boolean)*): Unit = {
  ...
  private def checkTableSource(
    tableEnv: TableEnvironment, tableName: String, expectToBeBounded: java.lang.Boolean): Unit = {
