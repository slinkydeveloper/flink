...
class StructuredToStringCastRule extends AbstractNullAwareCodeGeneratorCastRule<ArrayData, String> {
    ...
    protected String generateCodeBlockInternal(
        ...
        final CastRuleUtils.CodeWriter writer =
                new CastRuleUtils.CodeWriter().stmt(methodCall(builderTerm, "setLength", 0));

        if (inputStructuredType.getImplementationClass().isPresent()) {
            generateToStringConvertingToImplementationClass(
                    context, inputTerm, inputStructuredType, builderTerm, writer);
        } else {
            generateToStringUsingDefaultRepresentation(
                    context, inputTerm, inputStructuredType, builderTerm, writer);
        }
     ...
     */
    private void generateToStringUsingDefaultRepresentation(
            CodeGeneratorCastRule.Context context,
            String inputTerm,
            StructuredType inputStructuredType,
            String builderTerm,
    ...
    builder$283.setLength(0);
    java.lang.Object externalValue$285 = this.dataStructureConverter$2.toExternalOrNull(_myInput);
    if (externalValue$285 != null) {
        builder$283.append(externalValue$285.toString());
    }
    java.lang.String resultString$284;
    resultString$284 = builder$283.toString();
    result$1 = org.apache.flink.table.data.binary.BinaryStringData.fromString(resultString$284);

     */
    private void generateToStringConvertingToImplementationClass(
            CodeGeneratorCastRule.Context context,
            String inputTerm,
            StructuredType inputStructuredType,
            String builderTerm,
            CastRuleUtils.CodeWriter writer) {
        final String dataStructureConverterTerm =
                context.declareDataStructureConverter(inputStructuredType);
        final String externalValueTerm = newName("externalValue");

        // Convert to the user POJO and then invoke toString()
        writer.declStmt(
                        Object.class,
                        externalValueTerm,
                        methodCall(dataStructureConverterTerm, "toExternalOrNull", inputTerm))
                .ifStmt(
                        externalValueTerm + " != null",
                        thenBodyWriter ->
                                thenBodyWriter.stmt(
                                        methodCall(
                                                builderTerm,
                                                "append",
                                                methodCall(externalValueTerm, "toString"))));
    }
