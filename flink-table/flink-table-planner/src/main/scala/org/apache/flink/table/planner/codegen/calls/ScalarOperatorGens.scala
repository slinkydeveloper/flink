...
object ScalarOperatorGens {
  ...
  def generateCast(
    ...
    val resolutionResult = CastRuleProvider.resolve(operand.resultType, targetType)
    if (resolutionResult != null) {
      resolutionResult.getRule match {
        case codeGeneratorCastRule: CodeGeneratorCastRule[_, _] =>
          ...
          // Generate try-catch wrapping only if necessary
          if (resolutionResult.isFallible && nullOnFailure) {
    ...
    // Fallback to old isInteroperable logic FLINK-25930
  ...
  private def numericCasting(
    ...
    val rule = CastRuleProvider.resolveRule(operandType, resultType)
    rule match {
