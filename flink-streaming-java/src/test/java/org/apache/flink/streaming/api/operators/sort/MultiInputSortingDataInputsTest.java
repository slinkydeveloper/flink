        ...
        assertThat(collectingDataOutput.events)
                .isEqualTo(
                        Arrays.asList(
                                ...
                                new StreamRecord<>(99, 1), // max watermark from the preferred input
                                new Watermark(99L),
                                new StreamRecord<>(1, 1),
                                new StreamRecord<>(1, 2),
                                new StreamRecord<>(1, 3),
                                new StreamRecord<>(2, 1),
                                new StreamRecord<>(2, 2),
                                new StreamRecord<>(2, 3), // max watermark from the sorted input
                                Watermark.MAX_WATERMARK));
    }
        ...
        assertThat(collectingDataOutput.events)
                .isEqualTo(
                        Arrays.asList(
                                ...
                                new StreamRecord<>(2, 2),
                                new StreamRecord<>(2, 2),
                                new StreamRecord<>(2, 3), // max watermark from one of the inputs
                                Watermark.MAX_WATERMARK,
                                new StreamRecord<>(2, 3), // max watermark from the other input
                                Watermark.MAX_WATERMARK));
    }
    ...
    public void watermarkPropagation() throws Exception {
        ...
        assertThat(collectingDataOutput.events)
                .isEqualTo(
                        Arrays.asList(
                                new StreamRecord<>(0, 3),
                                new StreamRecord<>(1, 3), // watermark from the second input
                                new Watermark(3),
                                new StreamRecord<>(2, 3),
                                new StreamRecord<>(3, 3), // watermark from the first input
                                new Watermark(7)));
    }
