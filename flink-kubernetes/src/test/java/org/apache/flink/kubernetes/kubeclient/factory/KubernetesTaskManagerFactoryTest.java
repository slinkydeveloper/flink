...
public class KubernetesTaskManagerFactoryTest extends KubernetesTaskManagerTestBase {
    ...
    public void testPod() {
        ...
        assertThat(this.resultPod.getMetadata().getLabels().size()).isEqualTo(5);
        assertThat(this.resultPod.getSpec().getVolumes().size()).isEqualTo(4);
    }

    @Test
    public void testContainer() {
        final List<Container> resultContainers = this.resultPod.getSpec().getContainers();
        assertThat(resultContainers.size()).isEqualTo(1);
        ...
        assertThat(resultMainContainer.getEnv().size()).isEqualTo(4);
        assertThat(
                        resultMainContainer.getEnv().stream()
                                .anyMatch(envVar -> envVar.getName().equals("key1")))
                .isTrue();

        assertThat(resultMainContainer.getPorts().size()).isEqualTo(1);
        assertThat(resultMainContainer.getCommand().size()).isEqualTo(1);
        // The args list is [bash, -c, 'java -classpath $FLINK_CLASSPATH ...'].
        assertThat(resultMainContainer.getArgs().size()).isEqualTo(3);
        assertThat(resultMainContainer.getVolumeMounts().size()).isEqualTo(4);
    }
}
